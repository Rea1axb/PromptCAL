...
def log_accs_from_preds(y_true, y_pred, mask, eval_funcs: List[str], save_name: str, T: int=None, writer: SummaryWriter=None,
    ...
    for i, f_name in enumerate(eval_funcs):
        ...
        if writer is not None:
            writer.add_scalar(f'{f_name}/{log_name}_Old', old_acc, T)
            writer.add_scalar(f'{f_name}/{log_name}_New', new_acc, T)
            writer.add_scalar(f'{f_name}/{log_name}_All', all_acc, T)
